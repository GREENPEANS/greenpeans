...
export default React.createClass({
	...
	fetch(data) {
		 ...
		 * 当日按期还款金额
		 */
		for (let item in data.fifteenDaysOnTimeRepayMoney) {
		...
		for (var i = 0; i < item1_onTimeRepayMoney.length; i++) {
			data1_onTimeRepayMoney.push(data.fifteenDaysOnTimeRepayMoney[item2_onTimeRepayMoney[i]]);
		}
		/**
		 * s1(逾期1-10天)还款金额
		 */
		for (let item in data.fifteenDaysRepayMoneys1) {
		...
		for (var i = 0; i < item1_repayMoneys1.length; i++) {
			data1_repayMoneys1.push(data.fifteenDaysRepayMoneys1[item2_repayMoneys1[i]]);
		}
		/**
		 * s1(逾期大于10天)后还款金额
		 */
		for (let item in data.fifteenDaysRepayMoneyLaters1) {
		...
		for (var i = 0; i < item1_repayMoneyLaters1.length; i++) {
			data1_repayMoneyLaters1.push(data.fifteenDaysRepayMoneyLaters1[item2_repayMoneyLaters1[i]]);
		}
		/**
		 * 当日到期应还款金额
		 */
		for (let item in data.fifteenDaysNeedRepayMoney) {
		...
		for (var i = 0; i < item1_needRepayMoney.length; i++) {
			data1_needRepayMoney.push(data.fifteenDaysNeedRepayMoney[item2_needRepayMoney[i]]);
		}
		
		me.setState({
			data_onTimeRepayMoney: data1_onTimeRepayMoney,
			item_onTimeRepayMoney: item1_onTimeRepayMoney,
			data_repayMoneys1: data1_repayMoneys1,//s1(逾期1-10天)还款金额
			item_repayMoneys1: item1_repayMoneys1,//s1(逾期1-10天)还款金额
			data_repayMoneyLaters1: data1_repayMoneyLaters1,//s1(逾期大于10天)后还款金额
			item_repayMoneyLaters1: item1_repayMoneyLaters1,//s1(逾期大于10天)后还款金额
			data_needRepayMoney: data1_needRepayMoney,//当日到期应还款金额
			item_needRepayMoney: item1_needRepayMoney,//当日到期应还款金额
            first: false
		},() => {
            me.drawBar();
        });
	...
	drawBar() {
		...
		var option = {
			...
			legend: {
				...
				y: 25,
				itemGap: 10,
				data: ['当日按期还款金额','s1还款金额','s1后还款金额','当日到期应还款金额']
			},
			dataZoom: [{
		        type: 'slider',
		        start: 70,
