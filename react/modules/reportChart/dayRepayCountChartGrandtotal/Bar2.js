...
export default React.createClass({
	getInitialState() {
        return {
            ...
            data: [],
			item: [],
			data_onTimeRepayMoneyScale: [],//总按期还款比例
			item_onTimeRepayMoneyScale: [],//总按期还款比例
			data_repayMoneys1Scale: [],//总s1收回比例
			item_repayMoneys1Scale: [],//总s1收回比例
			data_repayMoneyLaters1Scale: [],//总s1后收回比例
			item_repayMoneyLaters1Scale: [],//总s1后收回比例
            first: true
        }
    },
	fetch(data) {
		 ...
		 * 总按期还款比例
		 */
		for (let item in data.fifteenDaysOnTimeRepayMoneyScalGrandtotale) {
		...
		for (var i = 0; i < item1_onTimeRepayMoneyScale.length; i++) {
			data1_onTimeRepayMoneyScale.push(data.fifteenDaysOnTimeRepayMoneyScalGrandtotale[item2_onTimeRepayMoneyScale[i]]);
		}
		/**
		 * 总s1收回比例
		 */
		for (let item in data.fifteenDaysRepayMoneys1ScaleGrandtotal) {
		...
		for (var i = 0; i < item1_repayMoneys1Scale.length; i++) {
			data1_repayMoneys1Scale.push(data.fifteenDaysRepayMoneys1ScaleGrandtotal[item2_repayMoneys1Scale[i]]);
		}
		/**
		 * 总s1后收回比例
		 */
		for (let item in data.fifteenDaysRepayMoneyLaters1ScaleGrandtotal) {
		...
		for (var i = 0; i < item1_repayMoneyLaters1Scale.length; i++) {
			data1_repayMoneyLaters1Scale.push(data.fifteenDaysRepayMoneyLaters1ScaleGrandtotal[item2_repayMoneyLaters1Scale[i]]);
		}
