...
export default React.createClass({
	...
	fetch(data) {
		 ...
		 * 全部认证比例
		 */
		for (let item in data.allAuthScale) {
		...
		for (var i = 0; i < item1_allScale.length; i++) {
			data1_allScale.push(data.allAuthScale[item2_allScale[i]]);
		}
		/**
		 * 芝麻信用比例
		 */
		for (let item in data.zhimaAuthScale) {
		...
		for (var i = 0; i < item1_zhimaScale.length; i++) {
			data1_zhimaScale.push(data.zhimaAuthScale[item2_zhimaScale[i]]);
		}
		/**
		 * 手机运营商比例
		 */
		for (let item in data.phoneAuthScale) {
		...
		for (var i = 0; i < item1_phoneScale.length; i++) {
			data1_phoneScale.push(data.phoneAuthScale[item2_phoneScale[i]]);
		}
		/**
		 * 银行卡比例
		 */
		for (let item in data.bankCardAuthScale) {
		...
		for (var i = 0; i < item1_bankCardScale.length; i++) {
			data1_bankCardScale.push(data.bankCardAuthScale[item2_bankCardScale[i]]);
		}
		/**
		 * 紧急联系人比例
		 */
		for (let item in data.contactAuthScale) {
		...
		for (var i = 0; i < item1_contactScale.length; i++) {
			data1_contactScale.push(data.contactAuthScale[item2_contactScale[i]]);
		}
		/**
		 * 实名认证比例
		 */
		for (let item in data.idAuthScale) {
		...
		for (var i = 0; i < item1_idScale.length; i++) {
			data1_idScale.push(data.idAuthScale[item2_idScale[i]]);
		}
	...
	render() {
		return <div id="bar2" style={{ height: '400px', width: '700px', margin: '0 auto'  }}></div>
	}
});